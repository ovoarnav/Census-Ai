...
def detect_mismatches(referral: ReferralExtract, facility: dict, payer_rules: Dict[str, dict]) -> List[MismatchFinding]:
    ...
    normalized = {
        "memory_text": " ".join([referral.cognitive_status or "", referral.behavioral_safety_concerns or ""]).lower(),
        "equipment_text": " ".join([referral.mobility_transfer_status or "", referral.durable_medical_equipment_needs or ""]).lower(),
        "airway_text": " ".join([
            referral.primary_diagnosis or "",
            referral.oxygen_respiratory_needs or "",
            referral.durable_medical_equipment_needs or "",
        ]).lower(),
        "meds_text": (referral.current_medications_or_mar or "").lower(),
        "allergies_text": (referral.allergies or "").lower(),
        "packet_missing_text": " ".join(referral.missing_or_unclear_items).lower(),
        "diagnosis_text": (referral.primary_diagnosis or "").lower(),
        "dialysis_text": (referral.dialysis_need or "").lower(),
        "infection_text": (referral.infection_isolation_status or "").lower(),
        "behavioral_text": (referral.behavioral_safety_concerns or "").lower(),
    }
    ...
    if contains(normalized["memory_text"], "wandering", "exit-seeking", "secured memory", "memory-care") and not caps.get("memory_care_locked_unit", False):
    ...
    if contains(normalized["equipment_text"], "bariatric") and not caps.get("bariatric_bed_available", False):
    ...
    if contains(normalized["airway_text"], "ventilator", "vent ") and not caps.get("ventilator", False):
    ...
    if contains(normalized["airway_text"], "tracheostomy", "trach") and not caps.get("tracheostomy_complex", False):
    ...
    if contains(normalized["dialysis_text"], "dialysis", "hemodialysis") and not caps.get("dialysis_on_site", False) and not caps.get("dialysis_transport_coordination", False):
    ...
    if contains(normalized["infection_text"], "contact", "isolation") and beds.get("isolation_private", 0) <= 0:
    ...
    if not normalized["meds_text"] or "not included" in normalized["meds_text"] or "referenced but" in normalized["meds_text"]:
    ...
    if not normalized["allergies_text"] or "not listed" in normalized["allergies_text"]:
    ...
    if ("stroke" in normalized["diagnosis_text"] or "dysphagia" in normalized["diagnosis_text"]) and ("swallow" in normalized["packet_missing_text"] or "diet" in normalized["packet_missing_text"]):
    ...
    if contains(normalized["infection_text"], "none", "no isolation") and normalized["packet_missing_text"].find("isolation") >= 0:
    ...
    if contains(normalized["behavioral_text"], "none", "no concerns") and contains(normalized["memory_text"], "aggressive", "unsafe", "exit-seeking"):
